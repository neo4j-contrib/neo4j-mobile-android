...
public class AutoConfigurator
{
    ...
    public AutoConfigurator( String dbPath, boolean useMemoryMapped, boolean dump ) {
    	this(null, dbPath, useMemoryMapped, dump);
    }
    
    public AutoConfigurator( Context context, String dbPath, boolean useMemoryMapped, boolean dump )
    {
        this.dbPath = dbPath;
        this.useMemoryMapped = useMemoryMapped;

        // determine physical machine memory
        // NOTE ahs: in Android we map it to the device's memory class (which is per application)
        if (context != null) {
        	ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        	totalPhysicalMemMb = am.getMemoryClass();
        }
        else {
